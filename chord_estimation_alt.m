...
function [d_root, type, rootname, typename r, R]=chord_estimation_alt(hpcp,chord,root, original_chord)
...
Nchords = 7;
for i=1:Nchords
    mean_profile(i) = mean(chord(i).altprof);
    var_profile(i) = var(chord(i).altprof);
end

R = zeros(13,hpcpsize);

for i=1:Nchords
    correlation(i) = sum((hpcp-mean_hpcp).*(chord(i).altprof-mean_profile(i)))/(var_hpcp*var_profile(i));
end


for i=1:(hpcpsize)
    for j=1:Nchords
        vector = [chord(j).altprof(hpcpsize-(i-1):hpcpsize) chord(j).altprof(1:hpcpsize-i)];
